...
export default async function validateProductId(req: Request, res: Response, next: NextFunction) {
  if (req.body.product_id) {
    try {
      ...
      const product = await db_connection.query(sql, [req.body.product_id]);
      db_connection.release();
      if (!product.rows[0]) return res.status(200).send({ message: productMessages.theresNoProductWithId(req.body.product_id) });
      next();
    } catch (error) {
